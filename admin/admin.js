...
var BRAND = (window.PosApp&&window.PosApp.brandConfig)||{business:"Mandala Sushi Caucel",marca:"mandala"};
var SESSION = BRAND.sessionKey || ((BRAND.marca||"mandala")+"Admin");
...
function abrirTurno(){
  ...
  checkTurno().then(function(existente){
    ...
    return fetch(base+"?marca=eq."+encodeURIComponent(BRAND.marca||"mandala")+"&usuario_id=eq."+encodeURIComponent(user.username)+"&select=*&order=abierto_en.desc&limit=1",{headers:headers}).then(function(r){if(!r.ok)throw new Error("No se pudo consultar el turno (HTTP "+r.status+")");return r.json()}).then(function(rows){
      var anterior=rows[0],ahora=new Date().toISOString(),datos={marca:BRAND.marca||"mandala",usuario_id:user.username,usuario_nombre:user.nombre||user.username,abierto_en:ahora,efectivo_inicial:ini,efectivo_final:0,estado:"abierto",cerrado_en:null},url=base,method="POST";
      if(anterior){method="PATCH";url=base+"?"+(anterior.id?"id=eq."+encodeURIComponent(anterior.id):"abierto_en=eq."+encodeURIComponent(anterior.abierto_en)+"&marca=eq."+encodeURIComponent(BRAND.marca||"mandala"))}
