...
var SESSION = "mandalaAdmin";
var BRAND = (window.PosApp&&window.PosApp.brandConfig)||{business:"Mandala Sushi Caucel",marca:"sakura"};
...
/* Nuevo pedido */
function nextFolio(){var max=0;state.orders.forEach(function(o){var n=parseInt(o.folio,10);if(n>max)max=n});var l=parseInt(localStorage.getItem("mandalaAdminFolio")||"0",10);if(l>max)max=l;var n=max+1;localStorage.setItem("mandalaAdminFolio",String(n));return("000"+String(n)).slice(-4);}
...
function PosReabrir(id){
  ...
  crypto.subtle.digest("SHA-256", enc.encode(pass)).then(function(buf){
    ...
    var log = JSON.parse(localStorage.getItem("mandalaReabrirLog")||"[]");
    log.push({folio:o.folio,orderId:id,usuario:user.nombre,username:user.username,fecha:new Date().toISOString()});
    if(log.length>200) log = log.slice(-200);
    localStorage.setItem("mandalaReabrirLog",JSON.stringify(log));
